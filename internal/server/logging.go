...
// PromptTokens 返回末帧 usage.prompt_tokens（缺失为 0）。
func (s *chatStatsReader) PromptTokens() int { return s.prompt }

// TotalTokens 返回末帧 usage.total_tokens（缺失为 0）。
func (s *chatStatsReader) TotalTokens() int { return s.total }

// statsCompletionTokens 返回末帧 usage.completion_tokens；usage 缺失时 -1
// （与 chatStat.toks 同口径，供 Stats.Record 区分"0 token"与"无 usage"）。
func statsCompletionTokens(s *chatStatsReader) int {
	if n, ok := s.Tokens(); ok {
		return n
	}
	return -1
...
func (s *chatStatsReader) parseSSELine(line string) {
	...
	s.total = chunk.Usage.TotalTokens
}
...
// usageTriple 从 Aggregate 返回的响应中提取 prompt/completion/total；缺失字段按 0，全缺返回 false。
func usageTriple(resp map[string]any) (prompt, completion, total int, ok bool) {
