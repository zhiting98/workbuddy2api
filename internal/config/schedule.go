...
type Schedule struct {
	...
	// TaskEnabled 显式关闭任务自动化排程（缺省 true；false = 停定时刷任务）。
	TaskEnabled bool `json:"task_enabled"`
	// TaskConcurrency 任务中心执行队列的并发度（按**账号**计，账号内仍严格串行）。
	//
	// 语义与边界：
	//   - 默认 10；上限 100（DefaultTaskConcurrencyMax），超过即钳到上限。
	//   - 0/负数 → 回落默认 10（"未配置"语义，与 *_enabled 的显式开关区分）。
	//   - 两条路径共用本值：看板「全部执行」与定时任务自动跑（TaskHours）。
	//   - 风控提示：并发 N 意味着同一时刻有 N 个账号在同一出口 IP 打上游；
	//     上游对短时间高频请求有风控，调高会提高账号被限流/封禁的风险，请自行权衡。
...
// 任务队列并发度的默认值与上限。
//
// 上限 100 而非无限：队列是"每账号一个 worker 逐个跑任务"，无上限会让
// 账号数直接决定瞬时并发；100 是留给大规模账号池的实操天花板，
// 同时避免把上游打到明显属于滥用的量级。
const (
	DefaultTaskConcurrency    = 10
	DefaultTaskConcurrencyMax = 100
)
...
func (s *Schedule) Normalize() error {
	...
	// 并发度：未配置（0/负）回落默认；超过上限钳到上限。
	// 注意这里是"纠正"而非报错——并发度是性能调优项，
	// 配错不该让整个进程起不来（与非法小时的处理风格不同，后者会跑错任务）。
	if s.TaskConcurrency <= 0 {
		s.TaskConcurrency = DefaultTaskConcurrency
	}
	if s.TaskConcurrency > DefaultTaskConcurrencyMax {
		s.TaskConcurrency = DefaultTaskConcurrencyMax
	}
